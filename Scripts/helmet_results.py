...
from datatypes.emme_scenario import Scenario, BikeScenario, DayScenario
...
class Results:
    def __init__(self, config):
        ...
        self.emme_scenarios = {
            "aht": Scenario(self.emme_project, first_scenario_id+2),
            "pt": Scenario(self.emme_project, first_scenario_id+3),
            "iht": Scenario(self.emme_project, first_scenario_id+4)
        }
        self.emme_project.logger.info("Emme-scenario instances created from {} to {}".format(first_scenario_id, first_scenario_id+4))
        self.zone_numbers = self.emme_scenarios["aht"].zone_numbers
    ...
    def assign(self):
        ...
        """ 
        assign_analysis.assign_auto_transit(self.emme_project, self.resultmatrices, self.emme_scenarios, self.bike_scenario)

    def auto_volumes_24h(self):
        """
        24h expansion of volumes on network.
        """
        for result_attribute in parameters.link_volumes:
            assign_analysis.auto_link_24h(self.emme_scenarios, self.day_scenario, result_attribute) 
            self.emme_project.logger.info("Sum 24h {} to scenario {}".format(result_attribute, self.day_scenario.scenario_id))

    def transit_volumes_24h(self):
        """
        24h expansion of volumes on network.
        """
        assign_analysis.transit_link_24h(self.emme_scenarios, self.day_scenario) 
        self.emme_project.logger.info("Sum 24h {} to scenario {}".format("transit volumes links", self.day_scenario.scenario_id))
        assign_analysis.transit_node_24h(self.emme_scenarios, self.day_scenario)
        self.emme_project.logger.info("Sum 24h {} to scenario {}".format("transit boardings", self.day_scenario.scenario_id))
        assign_analysis.transit_segment_24h(self.emme_scenarios, self.day_scenario)
        self.emme_project.logger.info("Sum 24h {} to scenario {}".format("transit volumes links", self.day_scenario.scenario_id))
    
    def bike_volumes_24h(self):
        """
        24h expansion of volumes on network.
        """
        assign_analysis.bike_link_24h(self.bike_scenario) 
        self.emme_project.logger.info("Sum 24h {} to scenario {}".format("bike_day", self.bike_scenario.scenario_id))
    ...
    def sum_omx_24h(self):
        ...
        Write to omx-files.
        """
        mtx_operations.mtx_to_24h(self.resultmatrices, self.zone_numbers, self.scenarios)
...
def main():
    ...
    # init logger
    logger = Log.get_instance().initialize(config, result_log=True)
    ...
    while True:
        ...
        print "3. Expand auto volumes to 24h"
        print "4. Expand transit volumes to 24h"
        print "5. Expand bike volumes to 24h"
        print "6. Read external count data"
        print "7. Get transit modes to omx-files"
        print "8. Get transfer transit trips to omx-files"
        print "9. Expand demand matrices to 24h (save to omx)"
        print "10. Aggregate omx-matrix results"
        print "99. Quit"
        try:
            ...
            if opt_chosen == 99:  
                ...
                results.auto_volumes_24h()
            elif opt_chosen == 4: 
                results.transit_volumes_24h()
            elif opt_chosen == 5:
                results.bike_volumes_24h()
            elif opt_chosen == 6: 
                ...
                print "Error: Invalid choice. Please select between 1-100."
        except ValueError:
