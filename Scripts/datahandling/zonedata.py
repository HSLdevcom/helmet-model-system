...
class ZoneData:
    def __init__(self, data_dir, zone_numbers):
        ...
        self.first_peripheral_zone = numpy.searchsorted(
            self.zone_numbers, peripheral[0])
        first_external = numpy.searchsorted(zone_numbers, external[0])
        self.first_external_zone = first_external
        self.external_zones = zone_numbers[first_external:]
        ...
        self._home_municipality = home_municipality.values
        self.read_data()

    def read_data(self, file_name_suffix=""):
        data_dir = self._data_dir
        popdata = read_csv_file(
            data_dir, ".pop"+file_name_suffix, self.zone_numbers, float)
        workdata = read_csv_file(
            data_dir, ".wrk"+file_name_suffix, self.zone_numbers, float)
        ...
        self.externalgrowth = read_csv_file(
            data_dir, ".ext", self.external_zones, float)
        ...
        pop = popdata["total"]
        self["population"] = pop
        first_peripheral = self.first_peripheral_zone
        ...
        self["population_own"] = self._home_municipality * pop.values
        self["population_other"] = (1-self._home_municipality) * pop.values
        self["workplaces_own"] = self._home_municipality * wp.values
        self["workplaces_other"] = (1-self._home_municipality) * wp.values
        self["service_own"] = self._home_municipality * serv.values
        self["service_other"] = (1-self._home_municipality) * serv.values
        self["shops_own"] = self._home_municipality * shop.values
        self["shops_other"] = (1-self._home_municipality) * shop.values
