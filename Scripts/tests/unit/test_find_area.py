...
class AreaFindTest(unittest.TestCase):
    def test_find_area(self):
        ...
        orig = 1007
        self.assertEqual(a.mapping[orig], "helsinki_other")
        dest = 17601
        self.assertEqual(a.mapping[dest], "peripheral")
        a.add(orig, dest)
        self.assertEqual(a.matrix.at["helsinki_other", "peripheral"], 1)
    ...
    def test_belongs_to_area(self):
        ...
        self.assertEqual(area, "helsinki_cbd")
        node.data3 = 245
        area = belongs_to_area(node)
        self.assertEqual(area, "surround_train")
