...
class TourCombinationModelTest(unittest.TestCase):
    def test_generation(self):
        ...
        self.assertAlmostEqual(sum(prob.values()), 1)
        prob = model.calc_prob("age_7-17", True, slice(0, 9))
        self.assertIs(type(prob[()]), pandas.Series)
        self.assertEqual(prob[("hw", "ho")].values.ndim, 1)
        self.assertEqual(prob[("hw", "hs")].values.shape[0], 9)
