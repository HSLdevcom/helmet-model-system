...
class Scenario():
    def __init__(self, emme_project, scenario_id, car_dist_cost=param.dist_unit_cost,
        ...
        """
        Class to hold methods for Emme-scenario (car, transit).
        """
    ...
    def get_auto_kms(self, attr, vdf):
        scen = self.emme_project.modeller.emmebank.scenario(self.scenario_id)
        network = scen.get_network()
        km = 0
        for link in network.links():
            link_vdf = link.volume_delay_func
            if link_vdf > 5:
                link_vdf = link_vdf - 5
            if link_vdf == vdf:
                km += link[param.link_volumes[attr]] * link.length
        return km
            
    def get_transit_kms(self, modes):
        scen = self.emme_project.modeller.emmebank.scenario(self.scenario_id)
        network = scen.get_network()
        transit_dist = 0
        for line in network.transit_lines():
            if line.mode.id in modes:
                for segment in line.segments():
                    transit_dist += ((60 / segment.line.headway)
                                    * segment.link.length)
        return transit_dist

    def get_transit_times(self, modes):
        scen = self.emme_project.modeller.emmebank.scenario(self.scenario_id)
        network = scen.get_network()
        transit_time = 0
        for line in network.transit_lines():
            if line.mode.id in modes:
                for segment in line.segments():
                    transit_time += ((60 / segment.line.headway)
