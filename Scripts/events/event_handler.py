...
class EventHandler(ModelSystemEventListener):
    ...
    def __init__(self, strict_mode=False):
        ...
        Args:
            ...
            strict_mode (bool): Whether to run in strict mode.
        """
        super().__init__()
        self.listeners = []
        self._create_methods(strict_mode)
    ...
    def _create_methods(self, strict_mode=False):
        ...
        for method_name in dir(ModelSystemEventListener):
            if method_name.startswith("on_") and callable(getattr(ModelSystemEventListener, method_name)):
                setattr(self, method_name, self._create_method(method_name, strict_mode))
                
    def _create_method(self, method_name, strict_mode=False):
